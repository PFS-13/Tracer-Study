...
def IndivRecWithSingleTask(Data, ColumnTypes={}, task=None,DataAsp=None, Num=10):
    if DataAsp is None:
        ...
        dataquery=DataAsp
    print("dquery ===", dataquery)
    # example of dataquery :
    #['num_rows(315).', 'fieldtype("Happiness Score",number).', 'cardinality("Happiness Score",299).', 'fieldtype("Happiness Rank",number).', 'cardinality("Happiness Rank",158).']
    header = list(ColumnTypes.keys())
    # example header : ['Happiness Score', 'Happiness Rank']
    ...
    # {
    #   'data': {'url': './values'}, 
    #    'encodings': [{'field': 'Happiness Score', 'type': 'quantitative'}, {'field': 'Happiness Rank', 'type': 'quantitative'}]
    # }
    
    #Mengubah spesifikasi visualisasi (format JSON / Vega-Lite-like) menjadi fakta ASP
    ...
    # query :  ['data("./values").', 'encoding(e0).', 'field(e0,"Happiness Score").', 'type(e0,quantitative).', 'encoding(e1).', 'field(e1,"Happiness Rank").', 'type(e1,quantitative).', 'task(comparison).']
    # dataquery : ['num_rows(315).', 'fieldtype("Happiness Score",number).', 'cardinality("Happiness Score",299).', 'fieldtype("Happiness Rank",number).', 'cardinality("Happiness Rank",158).']
    program = query + dataquery
    # example of program :

    # ['data("./values").', 'encoding(e0).', 'field(e0,"Happiness Score").', 'type(e0,quantitative).', 'encoding(e1).', 'field(e1,"Happiness Rank").',
    # 'type(e1,quantitative).', 'task(correlate).', 'num_rows(315).', 'fieldtype("Happiness Score",number).', 
    # 'cardinality("Happiness Score",299).', 'fieldtype("Happiness Rank",number).', 'cardinality("Happiness Rank",158).']
    ...
    # Menghapus duplikasi berdasarkan memori    
    ...
    #   'encoding': {'x': {'type': 'quantitative', 'field': 'Happiness Rank', 'bin': True}, 'y': {'type': 'quantitative', 'aggregate': 'count', 'scale': {'zero': False}}
    #                }
    # },
    # 'cost': 18.82, 
    # 'Ops': ['data("./values").', 'field(e0,"Happiness Rank").', 'field(e1,"Happiness Score").', 'type(e0,quantitative).', 'type(e1,quantitative).', 'channel(e0,x).', 'channel(5,y).', 'bin(e0,10).', 'aggregate(5,count).', 'type(5,quantitative).', 'mark(bar).'], 'fields': {'Happiness Rank'}, 'count': 0, 'task': set(), 'svg': []}        
...
def TaskVisAPIs(Data, ColumnTypes: List[dict] = [], task=None,DataAsp=None, Num=10, mode=1):
    ...
    print("TipeKolom==", ColumnTypes)
    print("DataTaskvis =", Data)
